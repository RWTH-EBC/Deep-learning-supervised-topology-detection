...
def calculate_metrics(y_true, y_pred, duration, y_true_val=None, y_pred_val=None):
    res = pd.DataFrame(data=np.zeros((1, 5), dtype=np.float), index=[0],
                       columns=['f1_score', 'precision', 'accuracy', 'recall', 'duration'])
...
#                                 'f1_score', 'precision', 'accuracy', 'recall', 'duration'])
...
def save_logs_t_leNet(output_directory, hist, y_pred, y_true, duration):
    ...
    if index_best_model is None:
        index_best_model = hist_df["accuracy"].idxmin()
